...
def colorize_feature(feature, cmap='viridis', pooling='max', vminp=0, vmaxp=100):
    """Converts a depth map to a color image.
    ...
    Returns:
        numpy.ndarray, dtype - uint8: Colored feature map. Shape: (H, W, 4)
    """
    if isinstance(feature, torch.Tensor):
        feature = feature.detach().cpu().numpy()

    feature = feature.squeeze()
    feature = np.max(feature, axis=0) if pooling == 'max' else np.mean(feature, axis=0)
    ...
    #     vmin = value.min()
    # else:
    vmin = np.percentile(feature,vminp)
    vmax = np.percentile(feature,vmaxp)
    
    if vmin != vmax:
        feature = (feature - vmin) / (vmax - vmin)  # vmin..vmax
    else:
        # Avoid 0-division
        feature = feature * 0.

    cmapper = matplotlib.cm.get_cmap(cmap)
    feature = cmapper(feature, bytes=True)  # (nxmx4)
