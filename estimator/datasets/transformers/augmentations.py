...
def aug_flip(image, depth_gt):
    do_flip = random.random()
    if do_flip > 0.5:
        image = copy.deepcopy(image[:, ::-1, :])  # 이미지 좌우 반전
        if isinstance(depth_gt, list):
            depth_gt_flipped = []
            for depth in depth_gt:
                if isinstance(depth, torch.Tensor):
                    depth_gt_flipped.append(depth.flip(dims=[-1]))  # PyTorch 텐서 반전
                else:
                    depth_gt_flipped.append(copy.deepcopy(depth[:, ::-1]))  # NumPy 또는 다른 형식
            depth_gt = depth_gt_flipped
        else:
            if isinstance(depth_gt, torch.Tensor):
                depth_gt = depth_gt.flip(dims=[-1])  # 단일 텐서 반전
            else:
                depth_gt = copy.deepcopy(depth_gt[:, ::-1])
    return image, depth_gt
