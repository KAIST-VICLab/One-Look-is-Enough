...
class UnrealStereo4kDataset_disp(Dataset):
    def __init__(
        self,
        mode,
        data_root, 
        ...
        self.random_split = getattr(self.transform_cfg, 'random_split', False)
    ...
    def __getitem__(self, idx):
        if self.random_split and idx % self.batch_size == 0:
            randint = random.randint(2,4)
            self.patch_split_num = (randint, randint)
        ...
        grid = torch.meshgrid(torch.linspace(-1, 1, int(3840)), torch.linspace(-1, 1, int(2160)), indexing="xy")
