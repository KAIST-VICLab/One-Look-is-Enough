...
def read_image_free(path, patch_split_num = (4,4)):
    img = cv2.imread(path)
    if img.ndim == 2:
    ...
    h, w, _ = img.shape
    h_ori, w_ori = h, w
    if w % patch_split_num[1] != 0:
        w = math.ceil(w / patch_split_num[1]) * patch_split_num[1]
    if h % patch_split_num[0] != 0:
        h = math.ceil(h / patch_split_num[0]) * patch_split_num[0]
    img = F.interpolate(torch.tensor(img).unsqueeze(dim=0).permute(0, 3, 1, 2), (h, w), mode='bicubic', align_corners=True)
    img = img.squeeze().permute(1, 2, 0).numpy()
    
    return img, (h_ori, w_ori)
        
    
                            
class Images:
    def __init__(self, root_dir, files, index, dataset_name, patch_split_num=(4,4)):
        self.root_dir = root_dir
        name = files[index]
        self.dataset_name = dataset_name
        self.rgb_image, self.res_original = read_image_free(os.path.join(self.root_dir, name), patch_split_num=patch_split_num)
...
class ImageDataset_res_free(UnrealStereo4kDataset):
    def __init__(
        self,
        rgb_image_dir,
        mode='',
        ...
        self.gt_dir = gt_dir
        self.dataset_name = 'general_res_free'
    ...
    def __getitem__(self, index):
        if self.gt_dir is not None:
            ...
            image = Images(self.rgb_image_dir, self.files, index, self.dataset_name, self.patch_split_num)
            image_rgb = image.rgb_image
            filename = image.name
            res_original = image.res_original
