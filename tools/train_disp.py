...
from estimator.trainer import Trainer_disp as Trainer
import shutil
import glob
...
def main():
    ...
    if runner_info.rank == 0 and cfg.debug == False:
        ...
        # Save the WandB run ID for resuming
        with open(osp.join(cfg.work_dir, "wandb_run_id.txt"), "w") as f:
    ...
    if getattr(cfg, 'val_dataloader_train', None) is not None:
        ...
        additional_val_dataloader = DataLoader(
            dataset,
            batch_size=1,
            shuffle=False,
            num_workers=cfg.val_dataloader_train.num_workers,
            pin_memory=True,
            persistent_workers=True,
            sampler=None)
    ...
    shutil.copy(args.config, osp.join(cfg.work_dir, 'config_run.py'))
    ...
    trainer = Trainer(
        ...
        model=model,
        additional_val_dataloader=additional_val_dataloader,
        )
