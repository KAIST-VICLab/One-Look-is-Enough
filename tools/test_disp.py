...
from estimator.tester import Tester_disp
...
def parse_args():
    ...
    parser.add_argument(
        '--model-config',
        type=str,
        default='./configs/PRO/model_config.py',
...
def main():
    ...
    if getattr(cfg, 'model', None) is None:
        cfg_tmp = Config.fromfile(args.model_config)
        cfg.model = cfg_tmp.model
    ...
    if args.test_type == 'consistency':
        ...
        cfg.val_dataloader.dataset.transform_cfg.network_process_size = list(cfg.model.config.patch_process_shape)
        ...
        cfg.general_dataloader.dataset.network_process_size = list(cfg.model.config.patch_process_shape)
        
        dataloader_config = cfg.general_dataloader
        dataset = build_dataset(cfg.general_dataloader.dataset)
    elif args.test_type == 'kitti':
        dataloader_config = cfg.kitti_dataloader
        dataset = build_dataset(cfg.kitti_dataloader.dataset)
    elif args.test_type == 'middle':
        cfg.val_dataloader_train.dataset.transform_cfg.network_process_size = list(cfg.model.config.patch_process_shape)
        cfg.val_dataloader_additional.dataset.transform_cfg.network_process_size = list(cfg.model.config.patch_process_shape)
        
        dataset_train = build_dataset(cfg.val_dataloader_train.dataset)
        dataset_additional = build_dataset(cfg.val_dataloader_additional.dataset)        
        dataset_train.patch_split_num = patch_split_num
        dataset_additional.patch_split_num = patch_split_num
        dataset = dataset_train + dataset_additional
        
        dataloader_config = cfg.val_dataloader_train
        
    else:
        dataloader_config = cfg.val_dataloader
        dataset = build_dataset(cfg.val_dataloader.dataset)
    
    #! dataset patch_split num 지정
    dataset.patch_split_num = patch_split_num
    ...
    runner_info.save_residual = args.save_residual
    ...
    print_log('Patch split num: {}, {}'.format(*patch_split_num), logger='current')
    print_log('cai mode:'+ args.cai_mode, logger='current')
    # print_log('network process size: {}, {}'.format(*cfg.val_dataloader.dataset.transform_cfg.network_process_size), logger='current')
    
    # build model
    if not hasattr(cfg, 'ckp_path') or not cfg.ckp_path:
        raise ValueError("Error: cfg.ckp_path is not set. Please provide a valid checkpoint path.")

    assert cfg.ckp_path.endswith('.pth'), f"Invalid checkpoint path: {cfg.ckp_path}"
    
    model = build_model(cfg.model)
    print_log('Checkpoint Path: {}. Loading from a local file'.format(cfg.ckp_path), logger='current')
    if hasattr(model, 'load_dict'):
        model.load_dict(torch.load(cfg.ckp_path)['model_state_dict'])
    ...
    tester.run(args.cai_mode, process_num=args.process_num, patch_split_num=patch_split_num)
